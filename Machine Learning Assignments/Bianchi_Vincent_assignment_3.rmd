...
bag_pred_class <- rep("nonTOR", nrow(bag_preds))
bag_pred_class[bag_preds[,-2]>= 0.5] <- 'TOR'

#Create predictions for bagging model - USE TEST

t <- table(bag_pred_class,test_db$label) # Create table with TEST
...
g1 <- ggplot(bst$evaluation_log, aes(iter, test_error_mean))+
...
best_pred <- predict(bst_final, dtest) # Create predictions for xgboost model

pred_dat <- cbind.data.frame(best_pred , test_db$label)

boost_pred_class <- rep('nonTOR', length(best_pred))
boost_pred_class[best_pred >= 0.5] <- 'TOR'


t <- table(boost_pred_class, test_db$label) # Create table
confusionMatrix(t, positive = "TOR") # Produce confusion
...
imp_mat$Feature[imp_mat$Importance>.05]

# Features with importance above .05
...
roc3 = roc(test_db$label, boost_preds_1)

plot.roc(roc1, print.auc = TRUE, col = "purple", print.auc.col = "purple")

plot.roc(roc2, print.auc = TRUE, print.auc.x = 0, print.auc.y = 0.6, col ="forestgreen", print.auc.col = "forestgreen", add = TRUE)

plot.roc(roc3, print.auc = TRUE, print.auc.x = 0, print.auc.y = 0.8, col ="#F2274C", print.auc.col = "#F2274C", add = TRUE)
...
The XGBoost model gave the best results out of the three models at .999
...
```{r}
set.seed(258506)
my_mdl <- xgboost(data = dtrain, # Set training data
              scale_pos_weight = (47828/6436), 
              eta = 0.3, # Set learning rate
              max.depth =  6, # Set max depth
              gamma = 0, # Set minimum loss reduction for split
...
boost_preds_check <- predict(my_mdl, dtest) # Create predictions for XGBoost model

# Convert predictions to classes, using optimal cut-off
boost_pred_class_cc <- rep("nonTOR", length(boost_preds_check))
boost_pred_class_cc[boost_preds_check >= 0.5] <- "TOR"


t_5 <- table(boost_pred_class_cc, test_db$label) # Create table
confusionMatrix(t_5, positive = "TOR")
```
