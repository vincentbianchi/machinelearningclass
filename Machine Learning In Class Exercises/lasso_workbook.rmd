...
```
```{r Visualize Missing Data}
t_bins <- bin(house_price$House.Price, nbins = 6, method = "length") # Bin response variable
plot_dat <- cbind.data.frame(t_bins, house_price[, feat_vars])
  gg_miss_fct(x = plot_dat, fct = t_bins) +
  labs(x = "House Prices")
```
...
```{r Linear Regression}
use_dat <- house_price[, c(2:12)] # Select the columns to use for analysis
fit_1 <- lm(House.Price ~., # Using ~. uses all other variables as explantory variables
            data = use_dat)
# Summarise regression
summary(fit_1)

# The variables that were significant (at the 5% level) were TIME, Education, Wages, Health Spending, Population, Tax Property, Unemployment Rate
```
```{r Lasso Model}
# Remove missing values
use_dat2 <- na.omit(use_dat)
# Scale explanatory variables
x_vars <- scale(use_dat2[,-1])

#  Fit lasso model
fit_3 <- glmnet(x = x_vars, # Fit explanatory variables
                y = use_dat2$House.Price, # Fit response variable
                alpha = 1, # Set alpha as 1 for lasso
                lambda = 0.5) # Set lambda as 0.5
