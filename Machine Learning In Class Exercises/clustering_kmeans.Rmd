...
From this we see that we have no missing values in the data. The variables correspond to twodifferent play types:
...
From this we can see that cluster 1 passed the ball very frequently but struggled at both passing and rushing, cluster 2 ran the ball very frequently and rarely passed the ball, cluster 3 was successful at both passing and rushing, cluster 4 ran the ball frequently but struggled at both passing and rushing. 
...
From this we see that the silhouette plot for the first clustering has issues with both cluster 1 and cluster 2. For the clustering with k=6 both cluster 6 appears to have samples that do not belong in that cluster. 
...
Lets check the distance between some teams to see how this looks. Lets focus on Georgia, Alabama, Notre, Stanford, and Army. We would expect that Georgia and Alabama are quite similar, that Notre Dame and Stanford are quite similar, that Notre Dame is reasonably similar to Georgia and that Army is quite distinct from the other teams. 

```{r Check distance}
cat("Georgia v Alabama")
dist(off_dat[off_dat$teams %in% c("Georgia", "Alabama" ),2:20])[1]
...
k_4_sol <- cbind.data.frame(off_dat$teams[off_dat$teams %in% c("Alabama", "Army", "Georgia",
                                                               "Notre Dame", "Stanford")],
                            clusters_1[off_dat$teams %in% c("Alabama", "Army", "Georgia",
                                                               "Notre Dame", "Stanford")])
...
k_6_sol <- cbind.data.frame(off_dat$teams[off_dat$teams %in% c("Alabama", "Army", "Georgia",
                                                               "Notre Dame", "Stanford")],
                            clusters_2[off_dat$teams %in% c("Alabama", "Army", "Georgia",
                                                               "Notre Dame", "Stanford")])
# Set names on new dataset
...
These look pretty good, however, it is interesting to note that Stanford, Georgia and Notre Dame end up in different clusters for K=6 even though they are quite similar to each other. This may indicate that we have too many clusters in the data if we want Notre Dame and Stanford to be in the same cluster, or that there are too few if we expect Notre Dame and Georgia/Alabama to be in different clusters. 
...
* Rush_avg_epa 
* Pass_avg_epa
* Rush_avg_epa_first_down
* Rush_avg_epa_second_down
* Pass_avg_epa_second_down
